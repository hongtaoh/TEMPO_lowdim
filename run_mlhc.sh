...
# 📦 Extract data
# ==============================================================================
DATA_TARBALL="/staging/hhao9/chil_lowdim_data.tar.gz"

if [[ -f "$DATA_TARBALL" ]]; then
    echo "📦 Extracting $DATA_TARBALL..."
    tar -xzf "$DATA_TARBALL"
    # If extraction creates "chil_lowdim_data", rename to "data"
    if [[ -d "chil_lowdim_data" ]]; then
        rm -rf data   # remove old data folder if it exists
        mv chil_lowdim_data data
        echo "Renamed chil_lowdim_data -> data"
    fi
else
